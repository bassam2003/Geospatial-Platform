...
module.exports = class AthenaExpress {
	...
	async query(event, context, callback)  {
		...
		var sqlQuery = "SELECT Feature.properties.title.'lang' AS title, Feature.properties.id AS id, Feature.properties.individualName AS individualName, Feature.properties.organisationName.'lang' AS organisationName, Feature.properties.published AS published, Feature.properties.description.'lang' AS description, Feature.properties.keyword.'lang' AS keyword, Feature.properties.topicCategory AS topicCategory, Feature.geometry.coordinates[0,0] as westBoundLongitude, Feature.geometry.coordinates[0,1] as southBoundLatitude, Feature.geometry.coordinates[1,0] as eastBoundLongitude, Feature.geometry.coordinates[2,1] as northBoundLatitude FROM cgp_metadata_lake WHERE keywords LIKE '%" + keyword + "%'";
		if (theme != null) {
			if (theme == "id")
				...
				sqlQuery = sqlQuery + " AND westBoundLongitude BETWEEN(Feature.geometry.coordinates[0,0] - 1, Feature.geometry.coordinates[0,0] + 1) AND  southBoundLatitude BETWEEN(Feature.geometry.coordinates[0,1] - 1, Feature.geometry.coordinates[0,1] + 1) AND  eastBoundLongitude BETWEEN(Feature.geometry.coordinates[1,0] - 1, Feature.geometry.coordinates[1,0] + 1) AND  northBoundLatitude BETWEEN(Feature.geometry.coordinates[2,1] - 1, Feature.geometry.coordinates[2,1] + 1)";
			}
