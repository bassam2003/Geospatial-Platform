...
module.exports = class AthenaExpress {
	...
	async query(event, context, callback)  {
		...
		let sqlQuery = "SELECT title_translated.en AS title, notes_translated.en AS notes FROM cgp_metadata_lake WHERE keywords LIKE '%" + keyword + "%'";
		
		if (theme != null) {
			if (theme == "Administration")
				sqlQuery = sqlQuery + " AND theme = 'Administration'";
			else if (theme == "Imagery")
				sqlQuery = sqlQuery + " AND theme = 'Imagery'";
		}
